...
class OmniRunnerDataPlane(OmniConnectorModelRunnerMixin):
    ...
    def _start_output_worker(self, *, max_pending_batches: int) -> None:
        ...
        self._native_output_worker: threading.Thread | None = threading.Thread(
    ...
    def _build_chunk_entries(
        ...
        *,
        ...
        if not getattr(self, "_can_send", True):
            receive_finished_ids = [req_id for req_id in sorted(terminal_req_ids) if req_id in self._native_requests]
            return [], receive_finished_ids
        payload_by_req = {

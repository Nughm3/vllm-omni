...
class TestWaitingForInputTransition(unittest.TestCase):
    ...
    def test_pending_input_registrations(self):
        ...
        req = _make_request("r1", status=RequestStatus.WAITING_FOR_INPUT)
        req.sender_info = {"host": "10.0.0.8", "zmq_port": 50251}
        waiting = MockQueue([req])
        ...
        self.assertEqual(len(coord.pending_input_registrations), 1)
        registration = coord.pending_input_registrations[0]
        self.assertEqual(registration.request_id, "r1")
        self.assertEqual(registration.external_req_id, "r1")
        self.assertEqual(registration.sender_info, req.sender_info)

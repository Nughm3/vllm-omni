...
class Orchestrator:
    ...
    async def _forward_to_llm_stage(
        ...
        *,
        ...
        try:
            span_name = f"PR2 before-old-manual: process_engine_inputs s{src_stage_id}->{next_logical}"
            with manual_span(span_name, request_id=req_id):
        ...
        for next_input in next_inputs:
            span_name = f"PR2 before-old-manual: submit_next_stage_request s{src_stage_id}->{next_logical}"
            with manual_span(span_name, request_id=req_id):
                with pr2_record_function(

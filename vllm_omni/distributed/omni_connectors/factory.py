...
def _create_composite_connector(config: dict[str, Any]) -> OmniConnectorBase:
    try:
        from .connectors.composite_connector import CompositeOmniConnector
    except ImportError:
        import sys

        sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))
        from omni_connectors.connectors.composite_connector import CompositeOmniConnector
    return CompositeOmniConnector(config)
...
OmniConnectorFactory.register_connector("CompositeOmniConnector", _create_composite_connector)
